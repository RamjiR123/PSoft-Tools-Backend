...
export async function backwardSympy(stmt: string, post: string): Promise<string> {
  const pyBinName = process.platform === "win32" ? "python.exe" : "python"; //python.exe
  const pythonBinaryPath = path.join(__dirname, "sympy-python", pyBinName); //python.exe path
  const scriptPath = path.join(__dirname, "Python-Files", "backward_sympy.py"); //backward_sympy path
  const projectRoot = "./";

  return new Promise<string>((resolve, reject) => {
    const cmd = `"${pythonBinaryPath}" "${scriptPath}" "${stmt}" "${post}"`;
    exec(cmd, { cwd: projectRoot }, (error, stdout, stderr) => {
      if (error) {
        console.error(`Backward SymPy execution error: ${error}`);
        console.error(`stdout: ${stdout}`);
        console.error(`stderr: ${stderr}`);
        return reject(stderr || error.message);
      }
      ...
      //wrap in braces like postcondition (formatting)
      const result = `{${javaExpr}}`;
      resolve(result);
    });
  });
